...
-- V3 reward funding: append-only source/program/allocation reference tables.
CREATE TABLE IF NOT EXISTS reward_funding_sources (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), source_type text NOT NULL CHECK(source_type IN ('PROTOCOL_REVENUE','SPONSOR','PARTNER','TREASURY','CREDIT')), name text NOT NULL, status text NOT NULL CHECK(status IN ('DRAFT','PENDING_APPROVAL','ACTIVE','PAUSED','EXHAUSTED','EXPIRED','CANCELLED')), currency text NOT NULL, approved_amount numeric(38,18) NOT NULL CHECK(approved_amount>=0), allocated_amount numeric(38,18) NOT NULL DEFAULT 0, reserved_amount numeric(38,18) NOT NULL DEFAULT 0, consumed_amount numeric(38,18) NOT NULL DEFAULT 0, starts_at timestamptz NOT NULL, expires_at timestamptz, created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now(), CHECK(allocated_amount<=approved_amount), CHECK(reserved_amount+consumed_amount<=allocated_amount));
CREATE TABLE IF NOT EXISTS reward_programs (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), name text NOT NULL, type text NOT NULL, status text NOT NULL CHECK(status IN ('DRAFT','ACTIVE','PAUSED','CLOSED','EXHAUSTED')), currency text NOT NULL, max_budget numeric(38,18) NOT NULL, accrued_liability numeric(38,18) NOT NULL DEFAULT 0, reserved_liability numeric(38,18) NOT NULL DEFAULT 0, claimed_liability numeric(38,18) NOT NULL DEFAULT 0, funding_priority jsonb NOT NULL, starts_at timestamptz, ends_at timestamptz, created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS reward_backing_allocations (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), funding_source_id uuid NOT NULL REFERENCES reward_funding_sources(id), reward_program_id uuid NOT NULL REFERENCES reward_programs(id), approved_amount numeric(38,18) NOT NULL, allocated_amount numeric(38,18) NOT NULL, reserved_amount numeric(38,18) NOT NULL DEFAULT 0, consumed_amount numeric(38,18) NOT NULL DEFAULT 0, created_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS reward_claim_reservations (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), reward_claim_id uuid REFERENCES reward_claims(id), amount_usd numeric(38,18) NOT NULL, status text NOT NULL CHECK(status IN ('active','released')), quote_expires_at timestamptz, created_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS funding_events (id bigserial PRIMARY KEY, funding_source_id uuid REFERENCES reward_funding_sources(id), event_type text NOT NULL, amount numeric(38,18) NOT NULL, metadata jsonb NOT NULL DEFAULT '{}'::jsonb, created_at timestamptz NOT NULL DEFAULT now());
